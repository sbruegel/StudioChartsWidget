  ...
  function acjsChart(tml3dRenderer, $timeout) {
      ...
      if (tml) {
        ...
        config.plugins = [{
          ...
          afterRender: (chart) => {
            ...
            scope.myCharts.push(chart.canvas.toDataURL());
          }
        }];
      }
      else {
    ...
    return {
      ...
      link: function(scope, element, attr) {
        ...
        if(attr.tml) {
          var canvasLow = scope._canvasLow = document.createElement("canvas");
          canvasLow.width = scope.canvasWidth/2;
          canvasLow.height = scope.canvasHeight/2;
          scope._chartLow = new Chart(canvasLow.getContext('2d'), scope._chartConfig);
        }
        var updateChart = () => {
          var data = scope.data;
        ...
        scope.$watch('myCharts', function(val) {
          //if(val[1] != undefined) {
            $timeout(function() { 
              let texture = val[0] + "&name=imgA " + val[1] + "&name=imgB";
              tml3dRenderer.setTexture(scope.imageId, texture);
              console.log(texture);
            
              $timeout(function() {
                console.log("test2");
                tml3dRenderer.setProperties(scope.widgetId, {shader: 'changeTex'});
              },100);
            },2000);
          //}
        });
